...
async function initializeOrchestrator(objective, overrides, userFeedback) {
    ...
    console.log(chalk_1.default.dim(`Loaded ${availableSkills.length} skills into the Orchestrator memory.`));
    // Health-checked provider list — Claudeson/Ollama only appear if reachable
    const availableProviders = await LLMFactory_1.LLMFactory.getAvailableProviders();
    ...
    (0, WebSocketServer_1.broadcastLog)('main', `[Round Table] Visionary: ${visionary.name} | Critic: ${critic.name} | Tactician: ${tactician.name}`);
    (0, WebSocketServer_1.broadcastLog)('main', `[Round Table] Debate Commencing...`);
    // 1. Draft Phase
    console.log(chalk_1.default.cyan(`[1/3] Asking ${visionary.name} for the initial architectural draft...`));
    (0, WebSocketServer_1.broadcastLog)('main', `[1/3] Asking ${visionary.name} for initial architectural draft...`);
    ...
    (0, WebSocketServer_1.broadcastLog)('main', `[Visionary / ${visionary.name}] Draft received (${draftResponse.length} chars).`);
    // 2. Critique Phase
    console.log(chalk_1.default.cyan(`[2/3] Passing the draft to ${critic.name} for critical review...`));
    (0, WebSocketServer_1.broadcastLog)('main', `[2/3] Passing draft to ${critic.name} for critical review...`);
    ...
    (0, WebSocketServer_1.broadcastLog)('main', `[Critic / ${critic.name}] Critique received (${critiqueResponse.length} chars).`);
    // 3. Synthesis Phase — Tactician synthesizes into strict JSON task list
    console.log(chalk_1.default.cyan(`[3/3] Asking ${tactician.name} to synthesize into parallel swarm tasks...`));
    (0, WebSocketServer_1.broadcastLog)('main', `[3/3] Asking ${tactician.name} to synthesize into JSON task list...`);
...
Critique:
${critiqueResponse}`;
    const synthesisResponse = await tactician.generateResponse(synthesisPrompt, 'You are a technical project manager. Output strict JSON only. No markdown fences.');
    let parsedData = { suggestions: [], tasks: [] };
    try {
        const jsonMatch = synthesisResponse.match(/\{[\s\S]*\}/);
        if (!jsonMatch)
            throw new Error('No JSON structure found in Tactician output');
        ...
        console.log(chalk_1.default.green(`✓ Tactician broke objective into ${parsedData.tasks.length} parallel tasks.`));
        (0, WebSocketServer_1.broadcastLog)('main', `✓ Tactician broke objective into ${parsedData.tasks.length} parallel tasks.`);
        parsedData.tasks.forEach((t, i) => {
            console.log(chalk_1.default.dim(`  ${i + 1}. [${t.provider}] ${t.name} → ${t.filename || t.command || '(instructions only)'}`));
            (0, WebSocketServer_1.broadcastLog)('main', `  ${i + 1}. [${t.provider}] ${t.name} → ${t.filename || t.command}`);
        });
    }
    catch (e) {
        console.log(chalk_1.default.red(`✗ Failed to parse Tactician output. Error: ${e}`));
        (0, WebSocketServer_1.broadcastLog)('main', `✗ Failed to parse Tactician output: ${e}`);
        console.log(chalk_1.default.dim('Raw Tactician response:\n' + synthesisResponse.substring(0, 1000)));
    }
    console.log(chalk_1.default.bold.magenta('\nDebate Complete. Waiting for human approval...\n'));
    (0, WebSocketServer_1.broadcastLog)('main', `\nDebate Complete. Plan generated. Waiting for human commander to review...`);
    return {
